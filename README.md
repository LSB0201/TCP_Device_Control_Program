# 📡 VEDA 원격 제어 시스템 (VedaRemoteControl)

본 프로젝트는 우분투 PC(Client)와 라즈베리파이(Server) 간의 지속 연결(Long-lived Connection)을 기반으로 하며, 하드웨어 모듈의 동적 로딩 및 epoll 멀티플렉싱을 활용한 웹/C-Client 원격 제어 시스템입니다.
...
`#define SERVER_IP "100.95.241.94" //" " 부분을 수정해주세요!`
...
# (만약 위 경로에서 파일을 찾을 수 없다고 나오면 아래 경로로 시도해 보세요)
